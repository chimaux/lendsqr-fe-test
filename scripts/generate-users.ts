...
const ORGANIZATIONS = [
  "Lendsqr","Irorun","Carbon","FairMoney","Renmoney","Branch","PalmPay","Kuda","Moniepoint"
];
const BANKS = [
  "Access Bank","GTBank","Zenith Bank","UBA","First Bank","Providus Bank","Fidelity Bank","Stanbic IBTC"
];
...
const LOAN_TYPES = ["Salary Loan","Business Loan","Car Loan","Mortgage","Personal Loan"];
const DOC_TYPES = ["Government ID","Passport","Driver License","Utility Bill"];
...
function guarantor(){
  return {
    ...
    relationship: pick(["Brother","Sister","Father","Mother","Friend"]),
...
function document(){
  return {
    ...
    document_name: pick([
      "National ID","International Passport","Driver License","Electricity Bill"
    ]),
...
function loan(){
  ...
  return {
    ...
    loan_amount: amount,
    interest_rate: faker.number.int({min:5,max:25}),
...
function transaction(){
  return {
    ...
    transaction_amount: faker.number.int({min:1000,max:500000}),
    transaction_description: pick(["Salary","Transfer","Savings","Loan Repayment"]),
...
function createUser(index:number){
  ...
  const minIncome = faker.number.int({min:100000,max:400000});
  const maxIncome = minIncome + faker.number.int({min:50000,max:500000});
  ...
  return {
    ...
    organization:{
      organization_id:`org_${faker.string.alphanumeric(6).toLowerCase()}`,
    ...
    general_details:{
      profile:{
        ...
        user_name: faker.internet.username({firstName:first,lastName:last}),
        full_name:`${first} ${last}`,
        first_name:first,
        last_name:last,
        email_address:faker.internet.email({firstName:first,lastName:last}),
        phone_number:generatePhoneNumber,
        ...
        user_tier:faker.number.int({min:1,max:3})
      },
      ...
      education_and_employment:{
        ...
        loan_repayment:faker.number.int({min:10000,max:150000})
      },
    ...
    bank_details:{
      ...
      currency:"NGN",
      balance:faker.number.int({min:10000,max:5000000}),
      available_balance:faker.number.int({min:5000,max:3000000}),
    ...
    savings:{
      wallet_balance:faker.number.int({min:1000,max:3000000}),
      locked_savings:faker.number.int({min:0,max:1000000}),
      target_savings:faker.number.int({min:100000,max:5000000}),
      interest_earned:faker.number.int({min:0,max:50000}),
      total_deposits:faker.number.int({min:10000,max:6000000}),
    ...
    app_and_system:{
      ...
      device_name:pick(["Windows 11","MacBook Pro","iPhone","Samsung Galaxy"]),
      browser:pick(["Chrome","Edge","Firefox","Safari"]),
      operating_system:pick(["Windows","macOS","Android","iOS"]),
...
const users = Array.from({length:500},(_,i)=>createUser(i));
